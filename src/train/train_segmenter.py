"""Entrenamiento del segmentador (§4.2) y comparacion SegFormer vs U-Net (§4.3).

Las dos corridas usan el mismo YAML cambiando solo `model.arch`, de modo que la
diferencia en Dice/IoU aisla el aporte del self-attention frente a un encoder
puramente convolucional:

    python -m src.train.train_segmenter --config configs/segmentation.yaml
    python -m src.train.train_segmenter --config configs/segmentation.yaml --arch unet

El subconjunto util son las ~2.594 imagenes con mascara de ISIC 2018 Task 1, filtradas
por `has_mask` en el CSV de splits. Conservan el mismo reparto por `lesion_id` que la
...
def run_epoch(model, loader, optimizer, scheduler, scaler, device, cfg, train: bool):
    model.train(train)
    ...
    with context:
        for images, masks in tqdm(loader, leave=False, desc="train" if train else "eval"):
            ...
            n += batch
            # Las metricas se calculan en FP32 para que el umbral no dependa del autocast.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Sobrescribe model.arch para la comparacion de §4.3",
    )
    ...
    print(f"Dispositivo: {device} | Arquitectura: {arch}")
    ...
    total_steps = cfg.train.epochs * len(loaders["train"])
    # Poly LR: el schedule estandar en segmentacion semantica, y el que usa el paper
    # original de SegFormer.
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer,
        lambda step: (1 - step / max(total_steps, 1)) ** cfg.train.poly_power,
    )
    scaler = (
    ...
    ckpt_path = models_dir / f"segmenter_{arch}_best.pt"
    ...
    for epoch in range(1, cfg.train.epochs + 1):
        tr_loss, tr_dice, tr_iou = run_epoch(
            model, loaders["train"], optimizer, scheduler, scaler, device, cfg, True
        )
        va_loss, va_dice, va_iou = run_epoch(
            model, loaders["val"], optimizer, scheduler, scaler, device, cfg, False
        )
    ...
    te_loss, te_dice, te_iou = run_epoch(
        model, loaders["test"], optimizer, scheduler, scaler, device, cfg, False
    )
    print(f"\n=== Test ({arch}) === Dice={te_dice:.4f} IoU={te_iou:.4f}")

    results_dir = resolve(cfg.paths.reports) / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    (results_dir / f"segmentation_{arch}.json").write_text(
        json.dumps(
            {
                "arch": arch,
                "n_params": n_params,
    ...
    print(f"Metricas en {results_dir / f'segmentation_{arch}.json'}")
